...
document.getElementById("getFile").onchange = function() {
      //retrieve information about the selected files
      ...
      fr.onload = function(){
            ...
            //store the text of te document, removing HTML tags
...
//alert the user to select a text
catch(err){
      window.alert(err);
}

function wordCloud(sourceText){
      //convert the souce text to lowercase
      //and remove laeding and trailing whitespace
      ...
      //reomove stop words from the text 
...
//create a 2 D array in which each item is an array
//containing a word and its duplicate count
let unique = [[words[0], 1]];

/// keep an index of the uniique words
let uniqueIndex = 0;
...
for (let i = 1; i < words.length; i++ ) {
      ...
      //find duplicates of the most repeated word
